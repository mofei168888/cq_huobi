...
class DataApi(object):
    ...
    def __init__(self):
        ...
        self.thread_db = Thread(target=self.onInsertdb)
    ...
    # -------------------------------"""订阅行情深度"""---------------------------------------
    def subscribeMarketDepth(self, symbol, type='step0'):
        '''
        :param symbol: $type 可选值：{ step0, step1, step2, step3, step4, step5, percent10 } （合并深度0-5）；step0时，不合并深度
        :return:
        '''
        topic = 'market.%s.depth.%s' % (symbol, type)
        self.subTopic(topic)
    ...
    # ------------------------------订阅K线数据-----------------------------
    def subscribeKline(self, symbol, period='1day'):
        '''
        :param symbol:
        :param period: $period 可选值：{ 1min, 5min, 15min, 30min, 60min, 4hour,1day, 1mon, 1week, 1year }
        :return:
        '''
        topic = 'market.%s.kline.%s' % (symbol, period)
        self.subTopic(topic)

    # ---------------------------------订阅所有交易对行情数据----------------------------
    def subscribeMarketTickers(self):
        ...
        while True:
            while not self.queue_data.empty():
                ...
                if 'depth.step' in data['ch']:
                    ...
                    for i in  range(len(data['tick']['data'])):#将int转型成字符串
                        data['tick']['data'][i]['id'] = str(data['tick']['data'][i]['id'])
                    self.db.insert('mt_trades', data)

                elif 'detail' in data['ch']:
                    self.db.insert('mt_market', data)

                elif 'kline' in data['ch']:
                    self.db.insert('mt_kline', data)

                elif 'tickers' in data['ch']:
                    self.db.insert('mt_tickers', data)
